...
def _build_styles() -> dict:
    return {
        ...
        "cover_title": ParagraphStyle(
            ...
            textColor=MEDICAL_BLUE, alignment=TA_CENTER, spaceAfter=10,  leading=32,
        ),
        ...
        "cover_label": ParagraphStyle(
            ...
            textColor=GREY_TEXT, alignment=TA_CENTER, spaceAfter=1, leading=12,
        ),
        "cover_value": ParagraphStyle(
            "cover_value", fontName="Helvetica", fontSize=11,
            textColor=DARK_TEXT, alignment=TA_CENTER, spaceAfter=8, leading=16,
        ),
        "cover_disclaimer": ParagraphStyle(
            "cover_disclaimer", fontName="Helvetica-Oblique", fontSize=7.5,
            textColor=GREY_TEXT, alignment=TA_CENTER, leading=11,
        ),
        "header_brand": ParagraphStyle(
            "header_brand", fontName="Helvetica-Bold", fontSize=11,
            textColor=WHITE, alignment=TA_CENTER,
        ),
        # Section & body
        "section_title": ParagraphStyle(
            "section_title", fontName="Helvetica-Bold", fontSize=12,
            textColor=MEDICAL_BLUE, spaceBefore=8, spaceAfter=4,
        ),
        "body": ParagraphStyle(
        ...
        "small": ParagraphStyle(
            ...
            textColor=GREY_TEXT, leading=12, spaceAfter=2,
        ),
        "small_italic": ParagraphStyle(
            "small_italic", fontName="Helvetica-Oblique", fontSize=8,
            textColor=GREY_TEXT, leading=12, spaceAfter=2,
        ),
        "bullet": ParagraphStyle(
            "bullet", fontName="Helvetica", fontSize=9,
            textColor=DARK_TEXT, leading=14, leftIndent=12, spaceAfter=3,
        ),
        # Table cells
        "th": ParagraphStyle(
        ...
        "rec_title": ParagraphStyle(
            "rec_title", fontName="Helvetica-Bold", fontSize=9.5,
            textColor=DARK_TEXT, leading=14, spaceAfter=2,
        ),
        "rec_reason": ParagraphStyle(
            "rec_reason", fontName="Helvetica-Oblique", fontSize=8.5,
            textColor=GREY_TEXT, leading=13, leftIndent=10, spaceAfter=4,
        ),
        "rec_foods": ParagraphStyle(
            "rec_foods", fontName="Helvetica", fontSize=8,
            textColor=DARK_TEXT, leading=12, leftIndent=10, spaceAfter=5,
        ),
        "urgency_text": ParagraphStyle(
            "urgency_text", fontName="Helvetica-Bold", fontSize=10,
            textColor=WHITE, alignment=TA_CENTER,
        ),
    }
...
class _MediScanDoc(BaseDocTemplate):
    ...
    def afterPage(self):
        ...
        canvas.drawRightString(
            PAGE_W - MARGIN, PAGE_H - HEADER_H + 4.5 * mm,
...
def _make_doc(path: str, patient_name: str) -> _MediScanDoc:
    ...
    content_frame = Frame(
        MARGIN, FOOTER_H + 4 * mm, CONTENT_W,
...
def _cover_page(s: dict, findings: ReportFindings, patient_name: str, generated_at: str) -> list:
    ...
    body_cell += [
        Spacer(1, 12 * mm),
        HRFlowable(width=80 * mm, thickness=0.5, color=colors.HexColor("#C8D8EE"), hAlign="CENTER"),
        Spacer(1, 8 * mm),
        Paragraph(
...
def _patient_section(s: dict, findings: ReportFindings) -> list:
    ...
    rows = [
        ...
        [Paragraph("Patient Name", s["td"]),      Paragraph(ctx.patient_name     or "Not Specified", s["td"])],
        [Paragraph("Report Type", s["td"]),       Paragraph(findings.report_type.replace("_", " ").title(), s["td"])],
        [Paragraph("Patient Age", s["td"]),       Paragraph(ctx.age              or "Not Specified", s["td"])],
        [Paragraph("Gender", s["td"]),            Paragraph(ctx.gender           or "Not Specified", s["td"])],
        [Paragraph("Report Date", s["td"]),       Paragraph(ctx.report_date      or "Not Specified", s["td"])],
        [Paragraph("Ordering Physician", s["td"]),Paragraph(ctx.ordering_physician or "Not Specified", s["td"])],
        [Paragraph("AI Confidence", s["td"]),     Paragraph(findings.confidence.title(), s["td"])],
    ]
    style = list(_BASE_STYLE)
    for i in range(1, len(rows)):
        style.append(("BACKGROUND", (0, i), (-1, i), LIGHT_BLUE if i % 2 == 0 else WHITE))
...
def _lab_values_section(s: dict, findings: ReportFindings) -> list:
    ...
    for i, lv in enumerate(findings.lab_values, 1):
        ...
        style.append(("BACKGROUND", (0, i), (2, i), LIGHT_BLUE if i % 2 == 0 else WHITE))
        ...
        for i, diet in enumerate(recs.dietary_recommendations, 1):
            ...
            if diet.foods_to_increase:
                block.append(Paragraph(
                    f"✅  Increase: {', '.join(diet.foods_to_increase)}", s["rec_foods"],
                ))
            if diet.foods_to_avoid:
                block.append(Paragraph(
                    f"❌  Reduce/Avoid: {', '.join(diet.foods_to_avoid)}", s["rec_foods"],
                ))
...
def generate_pdf(
    ...
    recommendations: ReportRecommendations,) -> str:
    """
    ...
    safe_seg     = (
        patient_name.replace(" ", "_").replace("·", "").replace("/", "-").strip("_")[:20]
        if patient_name != "Not Specified" else "Report"
    )
    ...
    try:
        ...
        story.append(HRFlowable(width=CONTENT_W, thickness=1, color=MEDICAL_BLUE, spaceAfter=6))
